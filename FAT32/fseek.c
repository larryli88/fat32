...
static int countio1 = 0; 
static int countio2 = 0;
...
void setClusterSecNum(cluster c) {
...
/*int main()
{
	...
	//printBootSectorInfo();
	/*
	 * Debug code for recoverDirEntry function
	 */
/*
	const char *path = "\\\\.\\F:\\recovery.jpg";
	recoverDirEntry(path);
	if (!((bool)0)) {
		printf("1");
	}
	
	/*
	 * Used to measure the time used by the searching function
	 */
	/*clock_t t;
	clock_t t2;
	double time_taken;
	...
	/*
	 * Debug code for setDirHashTable
	 */
	// Declare and initialize a new hashTable
	/*
	t2 = clock();
	fileEntry_t* hashTable[101];
	for (int i = 0; i < 101; i++) {
		hashTable[i] = (fileEntry_t*)malloc(sizeof(fileEntry_t));
		hashTable[i]->nextFile = NULL;
	}
	filePath_t* newPath = (filePath_t*)malloc(sizeof(filePath_t));
	newPath->nextPath = NULL;
	hashTable[101] = setDirHashTable(getRootDir(),hashTable,newPath);
	t2 = clock()-t2;*/


	/*
	 * Debug code for an item using Hashtable
	 */
	/*
	unsigned char search1[12] = "TARGET  SEA";
	search1[11] = '\0';
	printf("The directory of %s is :\n", search1);
	t = clock();
	int index = searchHashTable(hashTable, search1);
	if (index > 0) {
	...
	printf("IO:\t\t\t%d\n", countio1);
	printf("Item:\t\t\t%d\n", itemcount1);
	printf("Search function:\t\t\t%d\n", counta);
	t = clock() - t;
	time_taken = ((double)t) / CLOCKS_PER_SEC;
	setup_time = ((double)t2) / CLOCKS_PER_SEC;
	...
	*/
	 ...
	 */
	/*while (hashTable[index]->filePath->nextPath != NULL) {
		printf("%s/", hashTable[index]->filePath->path);
		hashTable[index]->filePath = hashTable[index]->filePath->nextPath;
	 ...
	 */
	 /*
	for (int i = 0; i < 101; i++) {
		while (hashTable[i]->nextFile != NULL) {
			printDirEntryInfo(hashTable[i]->fileInfo);
			hashTable[i] = hashTable[i]->nextFile;
		}
	}*/

	/*
	 * Debug code for delete dirEntry & writeSector & createDirEntry
	 */
	/*sector sect;
	sect.sectorNum = 24576;
	//deleteDirEntry(sect,1);
	
	unsigned char name[8] = "TEST    ";
	unsigned char ext[3] = "TXT";
	createDirEntry(sect,1,name,ext);
	
	unsigned char content[512] = "This is a test file...";

	const char *path = "\\\\.\\D:";
	WriteSector(path, 24608,content);*/

	/*
	 * print the whole directory and file of D:\\
	 */
	//printDirectory(getRootDir());

	/*
	 * Debug code for searching a dirItem using linked list
	 *//*
	printf("\n\n");
	
	t2 = clock();
	dirItem_t rootDir = setDirItem(getRootDir());
	t2 = clock() - t2;
	t = clock();
	unsigned char search[12] = "TARGET  SEA";
	search[11] = '\0';
	printf("The directory of %s is :\n", search);
	if (searchDirItem(rootDir, search)) {
		printf("\nFound!\n");
	}
	else {
		printf("The file doesn't exist!\n");
